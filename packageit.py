...
## TODO: update/mondernize NSIS installer ui
# copy(path(NSISSRC_PATH, 'gfx.nsi'), OUT_PATH)
# copy(path(NSISSRC_PATH, 'branding.nsi'), OUT_PATH)
# copy(path(NSISSRC_PATH, 'wizHeader.bmp'), OUT_PATH)
# copy(path(NSISSRC_PATH, 'wizHeaderRTL.bmp'), OUT_PATH)
# copy(path(NSISSRC_PATH, 'wizWatermark.bmp'), OUT_PATH)

copy(path(CONTENT_PATH, 'license.rtf'), OUT_PATH)
...
procfile(
    path(OUT_PATH, INSTALLER_NSI),
    lambda data: data.replace('@VERSION@', VERSION)
)
...
logsection(f'PACKING MozillaBuild v{VERSION} {C(DIM, GREEN, YELLOW+BGR, INV)} DONE! {C()}')
